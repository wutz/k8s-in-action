...
> * 其中 10.0.2.127/22 为 vip (**子网掩码值必须和物理网络一致**, 否则可能无法访问)，可以使用局域网中空闲的 IP，如果使用数据中心则需要联系管理员获取
...
curl -sfL https://rancher-mirror.rancher.cn/k3s/k3s-install.sh \
	| INSTALL_K3S_MIRROR=cn INSTALL_K3S_VERSION=v1.30.5+k3s1 sh -s - server \
    --cluster-init
...
> 对于生产环境，应当使用 `INSTALL_K3S_VERSION` 固定版本，版本信息可以从 [channel](https://update.k3s.io/v1-release/channels) 中查询
...
# 在非 mn 节点上加入集群
curl -sfL https://rancher-mirror.rancher.cn/k3s/k3s-install.sh \
	| INSTALL_K3S_MIRROR=cn INSTALL_K3S_VERSION=v1.30.5+k3s1 sh -s - agent \
	--server https://10.0.2.127:7443
```
